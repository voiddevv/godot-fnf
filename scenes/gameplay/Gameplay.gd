...
@onready var ratingdisplay = $HUD/ratingdisplay
...
func step_hit(step:int):
	...
	if (Conductor.position/1000.0 >= song_length):
		end_song()
...
func section_hit(section:int):
	for track in tracks:
		if abs((track.get_playback_position() * 1000.0) - (Conductor.position)) >= 20:
			resync_tracks()
...
func display_judgement(judgement:Judgement, tween:Tween):	
	...
	ratings_map[judgement.name.to_upper()] += 1
...
func update_score_text():
	ratingdisplay.text = "Sick: %s\nGood: %s\nBad: %s\nShit: %s" %[ratings_map.get("SICK",0),ratings_map.get("GOOD",0),ratings_map.get("BAD",0),ratings_map.get("SHIT",0)]
...
func _process(delta:float) -> void:
	...
	if health <= min_health and can_die:
		game_over()
	...
	if Input.is_action_just_pressed("ui_pause") and not Global.transitioning and can_pause:
		emit_signal("paused")
